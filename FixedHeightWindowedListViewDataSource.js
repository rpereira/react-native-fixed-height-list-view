...
class FixedHeightListViewDataSource {
   ...
   */
  getRowData(i) {
    let section = this.getParentSection(i);
    if (i == section.range[0]) { // First index of a section is always the header
      return { sectionId: section.sectionId };
    } else {
      return section.data[i - section.range[0] - 1];
    }
  }
  ...
  getRowHeight(i) {
    ...
    if (i == section.range[0]) { //first element is the section header
      return this.getSectionHeaderHeight(section.sectionId);
    } else {
  ...
  cloneWithCellsAndSections(dataBlob, sectionIds = Object.keys(dataBlob)) {
    ...
    sectionIds.forEach((sectionId) => {
      if (dataBlob[sectionId] && dataBlob[sectionId].length) {
        sectionIdsPresent.push(sectionId);
        this._rowCount += dataBlob[sectionId].length + 1; // 1 more for the section header
      }
    });
